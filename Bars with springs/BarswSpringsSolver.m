...
for count3 = 1:n_springs
    SK_matrix(:,:,count3) = SpringElementStiffness(k_vector(count3));   
end
K = zeros(n_nodes*2);
%Assemble
indx1=1;
...
for count5 = 1:n_springs
    K = SpringAssemble(K,SK_matrix(:,:,count5),I_matrix((indx1+count5),1),I_matrix((indx1+count5),2));
end
...
for count13 = 1:n_elements
    gg=1;
    S_InternalForces(:,:,count13) = K_matrix(:,:,count13)*[U_Vector(I_matrix(indx1+count13,gg));U_Vector(I_matrix(indx1+count13,gg+1))];
end